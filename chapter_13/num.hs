...
{- 在需要的地方添加括号。这个函数比较保守，有时候不需要也会加。Haskell 在构建 SymbolicManip 的时候已经处理好优先级了。-}
...
simplify x = x 
{- 新数据类型：Units。Units 类型包含一个数字和一个SymbolicManip，也就是计量单位。计量单位符号可以是 (Symbol "m") 这个样子。 -}
...
instance (Eq a, Floating a) => Floating (Units a) where
	...
	(Units xa ua) ** (Units xb ub) 
		...
		| otherwise = error "units for RHS of ** not supoeted"
